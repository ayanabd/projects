    ...
    -- - Создайте в MYSQL Workbench новую БД “NorthWind”
    -- На базе указанных таблиц, создайте таблицы с помощью команды CREATE TABLE. 
    -- Учтите типы данных и отношения между таблицами
    --  Загрузите данные с помощью команды INSERT (готовый файл прилагается)
    -- Выполните задания 1-15
    -- Напишите главный вывод вашего анализа
...
CREATE TABLE  Region (
    ...
    region_description VARCHAR(500) NOT NULL
);


CREATE TABLE  Shippers (
    shipper_id SMALLINT AUTO_INCREMENT PRIMARY KEY,
    company_name VARCHAR(40) NOT NULL,
    phone VARCHAR(24)
);
...
INSERT INTO NorthWind.US_States VALUES
    ...
    -- Задание 1 1. Определение объема продаж по категориям 
    -- Проанализировать, какие категории товаров приносят наибольшую выручку
    -- смотрим сколько выручки нам приносит каждый продукт
SELECT product_id, 
       SUM(unit_price * quantity * (1 - discount)) AS total_revenue
FROM Order_Details
GROUP BY product_id;

-- а здесь уже смотрим выручку по категориям
...
-- получаем промежуточный вывод, что категория 1 производит самое большее количество выручки
...
-- здесь узнаем что самое большое количество выручки приносят напитки, затем следуют молочные продукты
-- 1. Beverages
-- 2. Dairy Products
-- 3. Confections 
-- и замыкают тройку кондитерские изделия 


-- Задание 2
-- 2. Анализ сезонности продаж. Исследовать сезонные колебания продаж и выявить месяцы с 
-- наивысшими и наименьшими продажами.
-- Первым делом мы считаем количество проданных товаров по месяцам
-- Сначала извлекаем номер месяца 
-- затем группируем данные по месяцу и году 
-- считаем общее количество проданных товаров 
SELECT 
...
-- здесь мы уже находим месяц с максимальными продажами
SELECT 
...
FROM Orders o
JOIN Order_Details od ON o.order_id = od.order_id
GROUP BY year, best_month
ORDER BY total_sold DESC
LIMIT 1;  -- Показываем только 1 месяц с максимальными продажами
-- Месяц апрель показал самое максимальное количество продаж в 1998



-- здесь уже находим месяц с минимальными продажами
...
FROM Orders o
JOIN Order_Details od ON o.order_id = od.order_id
GROUP BY year, worst_month
ORDER BY total_sold ASC
LIMIT 1;  -- Показываем только 1 месяц с минимальными продажами
-- Месяц май показал самое минимальное количество продаж
...
-- сначала группируем общие продажи по номеру заказа
...
-- здесь используем join и группируем по работникам
...
-- получаем результат, что самый продуктивный работник это Margaret Peacock,
--  а самый наименее продуктивный работник это Steven Buchanan    
    
-- 4. Анализ повторных заказов 
-- Определить клиентов с наибольшим числом заказов, используя 
-- подзапросы для выявления значимых клиентов.

-- сначала считаем количество заказов для каждого клиента
SELECT customer_id, COUNT(order_id) AS total_orders
FROM Orders
GROUP BY customer_id
ORDER BY total_orders DESC;

-- теперь выбираем только тех клиентов, которые сделали максимальное количество заказов
...
-- получаем результат, что максимальное количество заказов сделал(-а) клиент Savea

-- 5. Анализ географического распределения продаж.  
-- Исследовать географическое распределение продаж и наиболее активные рынки.

SELECT  
    o.ship_country AS location, 
    SUM(od.unit_price * od.quantity * (1 - od.discount)) AS total_sales  
FROM Orders o
JOIN Order_Details od ON o.order_id = od.order_id  
GROUP BY o.ship_country  
ORDER BY total_sales DESC;  
-- наиболее активные по продажам показывает США, Германия и Австрия 

SELECT  
    o.ship_city AS location, 
    SUM(od.unit_price * od.quantity * (1 - od.discount)) AS total_sales  
FROM Orders o
JOIN Order_Details od ON o.order_id = od.order_id  
GROUP BY o.ship_city  
ORDER BY total_sales DESC;  
-- можно также рассмотреть продажи по городам
-- наибольшие продажи показывают Cunewalde, Graz, Boise
...
-- самую наивысшую маржинальность показывает товар под названием Côte de Blaye и наименьшую маржинальность показывает товар под названием Chocolade
...
FROM Order_Details
GROUP BY product_id;
-- узнаем сколько единиц каждого товара было продано 

SELECT product_id, product_name, units_in_stock
FROM Products;
-- узнаем сколько товаров осталось на складе 
...
SELECT 
    ...
    s.total_sold,  
    p.units_in_stock,
    s.total_sold / NULLIF(p.units_in_stock, 0) AS turnover_rate  
FROM Products p
JOIN (
    SELECT product_id, SUM(quantity) AS total_sold
    FROM Order_Details
...
-- узнаем что самые быстро продаваемый товар это Sir Rodney's Scones 
SELECT 
    p.product_name,  
    s.total_sold,  
    p.units_in_stock,
...
-- узнаем товар с самым низким оборотом это Grandma's Boysenberry Spread

-- 8. Динамика цен на товары  
-- Анализировать изменения цен на товары, используя подзапросы для 
-- сравнения текущих и прошлых цен.

SELECT 
    p.product_name,  
    p.unit_price AS current_price,  -- Текущая цена из Products
    (SELECT od.unit_price 
     FROM Order_Details od 
     JOIN Orders o ON od.order_id = o.order_id
     WHERE od.product_id = p.product_id 
     ORDER BY o.order_date ASC 
     LIMIT 1) AS first_price,  -- Самая старая цена
    ...
    (SELECT od.unit_price 
     ...
     LIMIT 1) AS last_price,  -- Самая последняя цена
    ...
    ( 
        ...
        (SELECT od.unit_price 
         ...
         ORDER BY o.order_date ASC 
         LIMIT 1)
    ) AS price_increase  -- Разница между последней и первой ценой
FROM Products p
ORDER BY price_increase DESC;  -- Сортировка по увеличению цены

/*
Как можно увидеть здесь
Товары с наибольшим увеличением цены 
1/ Côte de Blaye (разница $52.7) 
2/ Thüringer Rostbratwurst (разница $24.79) 
3/ Mishi Kobe Niku (разница $19.4) 

*/


-- 9. Анализ наименее продаваемых товаров 
-- Идентифицировать товары с низкой ротацией на складе, используя 
-- подзапросы для анализа минимальных продаж.
...
/*
-- товар с самой низкой ротацией на складе это товар под названием Mishi Kobe Nuku
*/


-- 10. Влияние скидок на продажи 
-- Проанализировать влияние скидок на общий объем продаж с 
-- использованием подзапросов для группировки данных по скидкам. 
...
SELECT 
    ...
    (SELECT SUM(unit_price * quantity) 
     FROM Order_Details 
     WHERE discount = 0) AS no_discount_sales
FROM Order_Details
GROUP BY discount
...
Анализируя представленные данные, можно сделать следующие выводы о влиянии скидок на общий объем продаж:

Без скидки (0%): Общий объем продаж составляет 750698.61, что является контрольным значением для сравнения.

При небольших скидках (1%-5%) наблюдается значительное снижение общего объема продаж, 
что может свидетельствовать о слабой реакции потребителей на небольшие скидки.
Например, при скидке 1% продажи составляют всего 29.7, а при скидке 5% – 147617.37.

Средние скидки (6-10%):
Продажи начинают восстанавливаться, но остаются ниже изначального уровня. 
Например, при скидке 10% продажи составляют 91499.14.
Это может свидетельствовать о том, что скидки в этом диапазоне все еще недостаточно 
привлекательны для значительного роста спроса.

Высокие скидки (15-25%):
Продажи продолжают расти, например, при скидке 25% объем продаж достигает 98938.57.
Это может указывать на то, что более высокие скидки начинают стимулировать спрос.

Итог:
Небольшие скидки (до 5%) не приводят к значительному увеличению продаж.
Средние скидки (6-10%) показывают некоторое улучшение, но объем продаж остается ниже бездисконтых значений.
Более высокие скидки (15-25%) начинают стимулировать спрос, но объем продаж все равно далек от уровня 
без скидок. Необходимо дальнейшее исследование, чтобы определить оптимальный размер скидки, 
который максимизирует доход, учитывая возможное снижение маржи.
*/


-- 11. Анализ удовлетворенности клиентов 
-- Оценить уровень удовлетворенности клиентов на основе частоты 
-- заказов и возвратов. 
...
SELECT 
    ...
    SUM(CASE WHEN o.shipped_date IS NULL THEN 1 ELSE 0 END) AS total_returns,
    (SUM(CASE WHEN o.shipped_date IS NULL THEN 1 ELSE 0 END) / COUNT(o.order_id)) * 100 AS return_rate,
    CASE 
        WHEN (SUM(CASE WHEN o.shipped_date IS NULL THEN 1 ELSE 0 END) / COUNT(o.order_id)) * 100 = 0 THEN 'очень высокий'
        WHEN (SUM(CASE WHEN o.shipped_date IS NULL THEN 1 ELSE 0 END) / COUNT(o.order_id)) * 100 <= 5 THEN 'высокий'
        WHEN (SUM(CASE WHEN o.shipped_date IS NULL THEN 1 ELSE 0 END) / COUNT(o.order_id)) * 100 <= 15 THEN 'средний'
        WHEN (SUM(CASE WHEN o.shipped_date IS NULL THEN 1 ELSE 0 END) / COUNT(o.order_id)) * 100 <= 30 THEN 'низкий'
        ELSE 'очень низкий'
...
Клиенты с уровнем удовлетворенности "Низкий" (например, RANCH, GREAL, CACTU, PERIC) 
имеют более высокий процент возвратов (от 16.67% до 20%), что может указывать на проблемы 
с доставкой или качеством продукции.

Клиенты с уровнем удовлетворенности "Средний" (например, LILAS, BLAUS, SIMOB, RICSU, RICAR, I INDD)
имеют более низкий процент возвратов (от 8.33% до 14.29%), что свидетельствует о более стабильном 
уровне удовлетворенности.

Общее количество заказов и возвратов:

Клиенты с большим количеством заказов, такие как LILAS (14 заказов) и I INDD (12 заказов), 
демонстрируют относительно низкий процент возвратов, что может говорить о их лояльности и 
удовлетворенности услугами.

Клиенты с меньшим количеством заказов, такие как RANCH (5 заказов) и CACTU (6 заказов), 
имеют более высокий процент возвратов, что может быть связано с меньшим опытом взаимодействия или 
специфическими проблемами.

Рекомендации:

Для клиентов с уровнем удовлетворенности "Низкий" рекомендуется провести анализ причин 
возвратов и улучшить качество обслуживания или продукции. Для клиентов с уровнем удовлетворенности "Средний" 
можно рассмотреть дополнительные меры по повышению лояльности, такие как программы лояльности или 
улучшение сервиса.
...
-- Разработать рекомендации для оптимизации управления запасами на 
-- основе анализа данных. 


SELECT p.product_name, p.units_in_stock, p.reorder_level, p.units_on_order,
       CASE 
           WHEN p.units_in_stock < p.reorder_level THEN 'Needs Reordering'
           WHEN p.units_in_stock = 0 THEN 'Out of Stock'
           ELSE 'Stock Level Sufficient'
       END AS stock_status
FROM products p
ORDER BY stock_status DESC;

/*
Текущая ситуация с запасами

Часть товаров имеет достаточный уровень запасов, но многие требуют пополнения ("Needs Reordering").
Некоторые позиции закончились на складе или находятся в процессе пополнения.
Рекомендации

Внедрить авто-контроль запасов для своевременного заказа.
Пересмотреть пороговые уровни для товаров с частым дефицитом.
Учитывать сезонные колебания спроса.
Это поможет снизить дефицит и оптимизировать закупки. 
...
SELECT 
    ...
    c.category_name,
    COUNT(o.order_id) AS total_orders,
    SUM(CASE WHEN o.shipped_date IS NULL THEN od.quantity ELSE 0 END) AS total_returns,
    COUNT(DISTINCT CASE WHEN o.shipped_date IS NULL THEN o.order_id ELSE NULL END) AS return_orders,
    ROUND((SUM(CASE WHEN o.shipped_date IS NULL THEN od.quantity ELSE 0 END) / SUM(od.quantity)) * 100, 2) AS return_rate,
    ROUND(AVG(od.unit_price), 2) AS avg_price,
    ROUND(AVG(od.quantity), 2) AS avg_quantity,
    CASE
        WHEN (SUM(CASE WHEN o.shipped_date IS NULL THEN od.quantity ELSE 0 END) / SUM(od.quantity)) * 100 IS NULL THEN 'No Returns'
        WHEN (SUM(CASE WHEN o.shipped_date IS NULL THEN od.quantity ELSE 0 END) / SUM(od.quantity)) * 100 <= 5 THEN 'Low'
        WHEN (SUM(CASE WHEN o.shipped_date IS NULL THEN od.quantity ELSE 0 END) / SUM(od.quantity)) * 100 <= 15 THEN 'Medium'
        WHEN (SUM(CASE WHEN o.shipped_date IS NULL THEN od.quantity ELSE 0 END) / SUM(od.quantity)) * 100 <= 30 THEN 'High'
        ELSE 'Very High'
...
Wimmers gute Semmelknödel (132 возврата, 17.84%)
Sasquatch Ale (90 возвратов, 17.79%)
Rössle Sauerkraut (98 возвратов, 15.31%)
Категории с наибольшими возвратами:
Grains/Cereals, Beverages, Produce, Confections
Возможные причины:
Качество (скоропортящиеся продукты)
Несоответствие ожиданиям (вкусовые предпочтения)
Логистика (повреждения при доставке)
Минимальные возвраты:
Некоторые товары (например, Boston Crab Meat, Côte de Blaye) не возвращают вовсе.

Вывод: Чаще всего возвращают напитки, кондитерские изделия и злаковые. Основные причины – качество, вкус и доставка.
*/



-- 14. Анализ географии 
-- Оцените эффективность продаж по географическим регионам и 
-- влияния территорий на производительность сотрудников. 

SELECT
    r.region_description AS region,
    COUNT(DISTINCT t.territory_id) AS num_territories,  
    ...
    ROUND(COUNT(o.order_id) / COUNT(DISTINCT e.employee_id), 2) AS orders_per_employee 
FROM
    Region r
JOIN
    Territories t ON r.region_id = t.region_id
LEFT JOIN  
    Employee_Territories et ON t.territory_id = et.territory_id
LEFT JOIN 
...
/*
Самый продуктивный регион – Western (наибольшие продажи на сотрудника – 807,624, наибольшее количество заказов на сотрудника – 1,300).
Southern также показывает высокие продажи на сотрудника (811,251), но с меньшим числом заказов.
Eastern имеет наибольшее число сотрудников и территорий, но среднюю производительность (682,549 продаж на сотрудника, 1,114 заказов).
Northern показывает наименьшую эффективность (524,302 продаж на сотрудника, 894 заказов).
Вывод: Наибольшая эффективность сотрудников в Western и Southern, несмотря на меньшее число территорий и сотрудников. Eastern компенсирует средней эффективностью за счет масштаба, а Northern отстает.
...
-- Исследуйте, как разные поставщики влияют на стоимость и доступность 
-- продуктов. Это может помочь в оптимизации закупок и возможно в 
-- переговорах о лучших условиях сотрудничества. 
...
SELECT
    ...
    ROUND(SUM(od.quantity * od.unit_price * (1 - od.discount)), 2) AS total_sales_value, 
    ...
    ROUND(AVG(p.reorder_level), 2) AS avg_reorder_level 
FROM
    Suppliers s
LEFT JOIN 
...
/*
Наибольшая производительность сотрудников в Западном и Южном регионах, где продажи на сотрудника превышают 800 тыс.
Восточный регион лидирует по количеству заказов, но имеет среднюю производительность.
Северный регион показывает наименьшую эффективность по продажам и заказам на сотрудника.
Чем больше число территорий в регионе, тем ниже средняя производительность сотрудников – возможно, требуется перераспределение ресурсов.
Краткий вывод по анализу поставщиков:
Некоторые поставщики предлагают низкие цены, но имеют высокие возвраты (например, Karkki Oy с возвратом 5,71%).
Высокая средняя цена не всегда коррелирует с высокими продажами – например, поставщик с самой высокой средней ценой (Aux joyeux ecclésiastiques) не является лидером по объему заказов.
Сроки доставки варьируются: у некоторых поставщиков они превышают 9 дней, что может повлиять на доступность товаров.
Оптимизация закупок возможна путем фокусировки на поставщиков с низкой возвратностью и приемлемыми сроками доставки.
*/


/*
Итоговый анализ данных NorthWind
1. Общие тенденции продаж
Категории с наибольшей выручкой: выделены ключевые категории продуктов, которые приносят максимальный доход. Они должны стать фокусом дальнейшего анализа и маркетинговых усилий.
Определены сезонные колебания спроса – обнаружены месяцы с пиковыми и минимальными продажами. Это позволяет компании скорректировать закупки и логистику.
2. Эффективность сотрудников
Выявлены самые продуктивные сотрудники, обработавшие наибольшее количество заказов.
Определены регионы, в которых сотрудники наиболее эффективны, что помогает оценить, какие рынки требуют дополнительных ресурсов.
3. Повторные заказы и лояльность клиентов
Найдены клиенты с наибольшим числом заказов, что может стать основой для программы лояльности.
Выявлены факторы, влияющие на удовлетворенность клиентов (повторные заказы, возвраты и жалобы).
4. Анализ товаров и складской логистики
Определены наиболее и наименее прибыльные товары, а также те, которые продаются медленно. Это важно для корректировки ассортимента.
Проведен анализ скорости оборота товаров – выявлены позиции с низкой оборачиваемостью, что позволяет оптимизировать складские запасы.
Оценено влияние скидок на продажи – скидки стимулируют продажи, но важно учитывать их влияние на прибыльность.
5. Географический анализ продаж
Выявлены наиболее активные рынки, что помогает сосредоточить маркетинговые усилия в нужных регионах.
Проведен анализ территорий и производительности сотрудников, чтобы определить точки роста.
6. Оптимизация работы с поставщиками
Проанализированы цены и доступность товаров у разных поставщиков, что позволит компании пересмотреть закупочные стратегии и добиться лучших условий сотрудничества.
Рекомендации для бизнеса
Фокус на прибыльные категории и товары – усиление маркетинга для топ-продуктов.
Коррекция ассортимента – снижение запасов товаров с низкой оборачиваемостью.
Сезонное планирование – закупки и скидки должны учитывать сезонные тренды.
Оптимизация работы с поставщиками – поиск лучших условий и сокращение затрат.
Развитие программы лояльности – удержание клиентов с высоким числом заказов.
Этот анализ дает компании инструменты для улучшения продаж, сокращения издержек и повышения эффективности работы.
